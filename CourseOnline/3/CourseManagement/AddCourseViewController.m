...
@interface AddCourseViewController ()<UIScrollViewDelegate,UITextViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate>
...
- (void)viewDidLoad {
    ...
    [rightNextBtn addTarget:self action:@selector(nextCourse) forControlEvents:UIControlEventTouchDown];
    [rightNextBtn setTitle:@"下一步" forState:UIControlStateNormal];
...
-(void)nextCourse{
    AddCourseContentViewController *addContentVC =[[AddCourseContentViewController alloc]init];
...
//    jieZhiChooseBtn.titleLabel.font =[UIFont systemFontOfSize:14];
    ...
    functionView.layer.borderWidth =1;
    functionView.layer.borderColor =[UIColor lightGrayColor].CGColor;
    ...
    [functionView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.size.mas_equalTo(CGSizeMake(KScreenWidth-520, 450));
    }];
    ...
    UIView *line =[[UIView alloc]initWithFrame:CGRectMake(0, 50, KScreenWidth-520, 1)];
    line.backgroundColor =[UIColor lightGrayColor];
    [functionView addSubview:line];
    

    [functionView addSubview:self.textView];
    [self.textView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(functionView.mas_top).offset(51);
        make.left.equalTo(functionView.mas_left);
...
-(void)choose1:(UIButton *)sender{
    ...
    [CGXPickerView showStringPickerWithTitle:@"自定义" DataSource:@[@"一二三", @"四五", @"六", @"七八", @"九十十一"] DefaultSelValue:@"六" IsAutoSelect:NO Manager:nil ResultBlock:^(id selectValue, id selectRow) {
        NSLog(@"%@",selectValue);
